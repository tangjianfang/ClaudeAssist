...
- 当前仓库名 `ClaudeAssist` 与项目品牌、GitHub Pages 地址、README 徽章与链接保持一致，**建议继续使用，不必修改**。
- `package.json` 中使用的包名 `claude-assist` 为 npm 常见小写命名规范，与仓库名大小写不同属于正常情况。
- 如未来确需改仓库名，建议同步修改 README 中所有 GitHub 链接、Pages 访问地址与克隆命令，避免跳转失效。
